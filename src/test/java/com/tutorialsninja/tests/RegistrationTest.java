...
import com.tutorialsninja.pages.EditAccountInfoPage;
import com.tutorialsninja.pages.HomePage;
import com.tutorialsninja.pages.LoginPage;
import com.tutorialsninja.pages.NewsletterPage;
...
public class RegistrationTest extends BaseTest {
	...
	public void verifyUserCanRegisterWithValidDetails() {
		...
		registerPage.registerUser(TestConstants.FIRST_NAME, TestConstants.LAST_NAME, TestUtils.generateNewEmail(),
				TestConstants.TELEPHONE, TestConstants.PASSWORD);
	...
	public void verifyUserReceivesAnEmailOfConfirmationAfterSuccessfulRegistration() {
		...
		registerPage.registerUser(TestConstants.FIRST_NAME, TestConstants.LAST_NAME, TestUtils.generateNewEmail(),
				TestConstants.TELEPHONE, TestConstants.PASSWORD, true);
	...
	public void verifyUserCanRegisterWithNewsletterSubscriptionNo() {
		...
		registerPage.registerUser(TestConstants.FIRST_NAME, TestConstants.LAST_NAME, TestUtils.generateNewEmail(),
				TestConstants.TELEPHONE, TestConstants.PASSWORD, false);
	...
	public void verifyRegistrationFailsWhenPasswordsDoNotMatch() {
		...
		registerPage.registerUser(TestConstants.FIRST_NAME, TestConstants.LAST_NAME, TestUtils.generateNewEmail(),
				TestConstants.TELEPHONE, TestConstants.PASSWORD, "678u90");
	...
	public void verifyRegistrationFailsByProvidingExistingDetails() {
		...
		registerPage.registerUser(TestConstants.FIRST_NAME, TestConstants.LAST_NAME, TestConstants.EXISTING_EMAIL,
				TestConstants.TELEPHONE, TestConstants.PASSWORD);
	...
	public void verifyRegistrationFailsByProvidingInvalidEmail() {
		...
		for (String invalidEmail : TestConstants.INVALID_EMAIL) {
			...
			registerPage.registerUser(TestConstants.FIRST_NAME, TestConstants.LAST_NAME, invalidEmail,
					TestConstants.TELEPHONE, TestConstants.PASSWORD, true);
	...
	public void verifyRegistrationFailsByProvidingInvalidTelephoneNumber() {
		...
		for (String invalidTelephoneNumber : TestConstants.INVALID_TELEPHONE_NUMBER) {
			...
			registerPage.registerUser(TestConstants.FIRST_NAME, TestConstants.LAST_NAME, TestUtils.generateNewEmail(),
					invalidTelephoneNumber, TestConstants.PASSWORD, true);
			...
			// if telephone number > 3 digits but invalid so warning expected + registration
			// fails
			// but this is a bug, the application allows successful registration even after
			// entering an invalid phone number.
			else {
				Assert.assertTrue(successPage.isSuccessPageDisplayed());
				System.out.println(
						"BUG : Registration succeeded for invalid telephone number : " + invalidTelephoneNumber);
	...
	public void verifyRegistrationFailsByProvidingOnlySpacesIntoMandatoryFields() {
		...
		if (!browserValidationMessage.isEmpty()) {
			...
			Assert.assertEquals(uiValidationMessage, "E-Mail Address does not appear to be valid!");
		...
		registerPage.registerUser(multipleSpaces, multipleSpaces, TestUtils.generateNewEmail(), multipleSpaces,
				multipleSpaces, true);
	...
	public void verifyPasswordComplexityStandardsOnRegisterPage() {
		...
		for (String invalidPassword : TestConstants.INVALID_PASSWORD) {
			...
			registerPage.registerUser(TestConstants.FIRST_NAME, TestConstants.LAST_NAME, TestUtils.generateNewEmail(),
					TestConstants.TELEPHONE, invalidPassword, true);
			...
			if (successPage.isSuccessPageDisplayed()) {
				...
				Assert.assertTrue(invalidPasswordWarningMessage != null && invalidPasswordWarningMessage.length() > 0,
						"Expected some password validation message");
			}
		}
	...
	public void verifyLeadingAndTrailingSpacesAreTrimmedInRegisterFields() {
		...
		Assert.assertTrue(editAccountInfoPage.isEditInformationPageDisplayed(),
				"EditInformation page is not displayed");

		String actualFirstName = editAccountInfoPage.getFirstNameInputFieldValue();
		String actualLastName = editAccountInfoPage.getLastNameInputFieldValue();
		String actualEmail = editAccountInfoPage.getEmailInputFieldValue();
		String actualTelephone = editAccountInfoPage.getTelephoneFieldValue();
...
//		System.out.println("Actual: [" + actualFirstName + "]");
		...
		if (!actualFirstName.equals((firstName).trim()) || !actualLastName.equals(lastName.trim())
				|| !actualEmail.equals(email.trim()) || !actualTelephone.equals(telephone.trim())) {
	...
	public void verifyRegistrationFailsWhenPrivacyPolicyCheckboxIsNotSelected() {
		...
		registerPage.registerUserWithoutClickingPrivacyPolicyCheckbox(TestConstants.FIRST_NAME, TestConstants.LAST_NAME,
				TestUtils.generateNewEmail(), TestConstants.TELEPHONE, TestConstants.PASSWORD, true);
	...
	public void verifyPasswordAndConfirmPasswordInputsAreHiddenOnRegisterPage() {
		...
		Assert.assertEquals(actualPasswordFieldType, "password",
				"Password field is not hidden (type should be 'password')");

		Assert.assertEquals(actualConfirmPasswordFieldType, "password",
				"Confirm Password field is not hidden (type should be 'password')");
	}
	...
	public void verifyNavigationToDifferentPagesFromRegisterPage() {
		...
		for (Map.Entry<String, String> entry : registerPageNavigationLinks.entrySet()) {
			...
			try {
				...
				} else if (link.equals("Desktops") || link.equals("Laptops & Notebooks") || link.equals("Components")
						|| link.equals("MP3 Players")) {
					registerPage.hoverAndClickHeaderLink(link);

				} else if (link.equals("Tablets") || link.equals("Software") || link.equals("Phones & PDAs")
						|| link.equals("Cameras")) {
	...
	public void verifyRegistrationFailsWhenPasswordConfirmFieldIsBlank() {
		...
		try {
			...
			Assert.assertTrue(
					actualConfirmPasswordWarningMessage.contains(TestConstants.EXPECTED_CONFIRM_PASSWORD_WARNING),
					"Confirm Password warning message is not displayed or incorrect");
	...
	public void verifyRegisterPageUIElements() {
		...
		try {
			// Page Heading
			Assert.assertEquals(registerPage.getPageHeading(), TestConstants.EXPECTED_REGISTER_PAGE_HEADING,
					"Page Heading mismatch");

			// Page Title
			Assert.assertTrue(registerPage.getPageTitle().contains(TestConstants.EXPECTED_REGISTER_PAGE_TITLE),
					"Page Title mismatch");

			// Page URL
			Assert.assertTrue(registerPage.getPageURL().contains(TestConstants.EXPECTED_REGISTER_PAGE_URL),
					"Page URL mismatch");

			// Breadcrumb
			Assert.assertTrue(registerPage.getBreadcrumb().contains(TestConstants.EXPECTED_REGISTER_PAGE_BREADCRUMB),
					"Breadcrumb mismatch");
	...
	public void verifyRegisterPageUICompletely() {
		...
		try {
			...
			Assert.assertTrue(registerPage.isRegisterPageRightColumnForgotPasswordDisplayed(),
			...
			Assert.assertTrue(registerPage.isRegisterPageFooterInformationSectionDisplayed(),
					"Information section missing");
			Assert.assertTrue(registerPage.isRegisterPageFooterCustomerServiceSectionDisplayed(),
					"Customer Service section missing");
			Assert.assertTrue(registerPage.isRegisterPageFooterExtrasSectionDisplayed(), "Extras section missing");
			Assert.assertTrue(registerPage.isRegisterPageFooterMyAccountSectionDisplayed(),
					"Footer My Account section missing");
	...
	public void verifyUserCanRegisterUsingKeyboardKeys() {
		...
		registerPage.registerUserUsingKeyboardKeys(TestConstants.FIRST_NAME, TestConstants.LAST_NAME,
				TestUtils.generateNewEmail(), TestConstants.TELEPHONE, TestConstants.PASSWORD, true);
